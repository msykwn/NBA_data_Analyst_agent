...
def _current_season() -> str:
    ...
    NBAシーズンは10月開幕・6月頃終了のため:
    - 10〜12月: 当年開幕のシーズン(例: 2025-10月 → "2025-26")
    - 1〜9月: 前年開幕のシーズン(例: 2026-06月 → "2025-26")
    ただし開幕前(7〜9月)はオフシーズンのため、前シーズン("2024-25")を返す。
    実質的に 10〜6月 は当年または前年、7〜9月 は2年前開幕シーズンを返す。
    シンプルに: 7月以降の場合のみ year-1 し、それ以外は year-1 で統一する。
    NBAシーズンとして有効なデータが存在するのは 10月〜翌6月 のため、
    7〜9月(オフシーズン)は直前に終わったシーズンを返す。
    """
    today = date.today()
    # 10月以降はその年開幕シーズン、それ以外(1〜9月)は前年開幕シーズン
    # ただし7〜9月はオフシーズンで前シーズンのデータしかないため同じ式で問題なし
