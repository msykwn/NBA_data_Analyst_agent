...
[AWS Bedrock AgentCore Runtime]  … エージェントのホスティング・実行基盤
        ├─ Gateway  … NBAデータ取得用の外部APIをエージェントから呼び出せるツールに変換
...
- **Gateway**: 外部REST API(NBAデータ取得API)をMCP互換のツールへ変換し、エージェントから呼び出し可能にする。
...
3. 動作確認ができてから、Gateway経由のNBAデータ取得ツール、Memoryによる会話履歴管理を順次組み込む
