...
### 4.2 State管理: S3によるリモート管理(stateロックはS3ネイティブの `use_lockfile` を使用)

Terraformのstateは、**S3バケット(state本体の保存 + state lockingの両方)** によるリモートバックエンドで管理する。stateロックには、Terraform 1.10+で導入されたS3ネイティブの `use_lockfile` を用いる(DynamoDBは使用しない)。

- **理由**: ローカルstateと比べ、複数端末からの利用やstate紛失リスクに備えられるリモートstate管理を実践する価値が高い。ロックについても、S3単体で完結する `use_lockfile` の方が管理対象リソースが少なく構成がシンプルになる
- **既知のブートストラップ問題と対応方針**: state管理用のS3バケット自体をTerraformで管理しようとすると「stateを置く場所がまだ存在しない」という鶏卵問題が生じる。このリソースは、Terraform管理外(AWS CLIで手動作成)で先に用意し、以降のリソースをS3 backend経由で管理する方針とする
- **実施内容**(2026-06-09): AWS CLIで以下を作成済み
  - S3バケット: `nba-data-analyst-agent-tfstate-335541954164`(リージョン: ap-northeast-1。バージョニング有効化、サーバーサイド暗号化(AES256)、パブリックアクセスブロックを設定)
  - `terraform/backend.tf` で `use_lockfile = true` を指定し、`terraform init` / `terraform plan` でS3バックエンドとの疎通を確認済み
...
- **対象となるロール・ユーザー**:
  - **AgentCore Runtime 実行ロール**: Bedrock(Claude)呼び出し、AgentCore Memory操作、CloudWatch Logsへの出力など、エージェントの動作に必要な権限のみを付与する
  - **Terraform実行ユーザー/ロール**: インフラ構築・変更に必要なリソース(IAM、AgentCore Memory、S3、DynamoDB、Budgets等)の管理権限を付与する。個人学習用途のため、開発者個人のIAMユーザーに必要な権限をアタッチする運用を想定する
- 各ロールの具体的なポリシー(許可するアクション・リソースの範囲)は、実装段階で必要な操作を洗い出しながら設計する

## 6. 認証情報管理

`spec/architecture.md` 7節の通り、データ取得元として採用した stats.nba.com / `nba_api` はAPIキー等の認証を必要としないため、本フェーズでは外部NBA API向けの認証情報管理(Secrets Manager等)は**不要**である。

- 将来的に認証が必要なデータソース・外部サービスを追加する場合は、AWS Secrets Managerでの管理を改めて検討する。
...
1. **ブートストラップ**(手動・1回限り): Terraform state管理用のS3バケットを作成する(4.2節)。実施済み
2. **インフラ構築**(Terraform): IAMロール、AgentCore Memory、Budgets等のAWSリソースを `terraform apply` で構築する
...
実装過程で生じた判断・変更は、本書または関連する個別要件ファイルに都度追記していく。(stateロック方式の変更経緯は4.2節を参照)
