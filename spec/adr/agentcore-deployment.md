...
### 2026-06-10: nba_api が AgentCore Runtime 上で動作しないことが判明

**変更前の想定:**
- `nba_api` ライブラリ（`stats.nba.com` へアクセス）をそのまま AgentCore Runtime 上で使用する

**発覚した問題:**
- デプロイ・起動自体は成功したが、実際に呼び出すと `stats.nba.com` への HTTP リクエストが毎回 `ReadTimeout`（30秒）でタイムアウトする
- ローカル環境（自宅 IP）では同じコードが正常に動作するため、**AWS のパブリック IP からのアクセスを `stats.nba.com` がブロックしている**と判断
- `nba_api` はブラウザ風 User-Agent を付与しているが、それでも AWS IP からはタイムアウトする

**検討した回避策と却下理由:**

| 案 | 却下理由 |
|---|---|
| NAT Gateway で固定 IP 経由にする | AWS IP レンジ全体がブロックされている可能性が高く、固定 IP にしても根本解決にならない。コスト増にもなる |
| balldontlie API に切り替える | シーズン平均スタッツ・順位など主要機能が GOAT プラン（$39.99/月）必須で、学習目的のプロジェクトにはコスト過多 |
| AgentCore Runtime を諦めてローカル/EC2 で動かす | AgentCore の学習という本来の目的を達成できなくなる |

**採用した方針:**
- **AgentCore Runtime はそのまま使い、データソースを AWS サービス内で完結する形に切り替える**
- `nba_api` への依存を除去し、AgentCore Runtime からアクセス可能な別データソースに置き換える
- ツールのインターフェース（関数名・引数・戻り値の構造）は変えず、バックエンドの実装だけ差し替えられる設計にする（既存の `nba_api` 実装とも切り替え可能にする）
- 具体的なデータソースの選定・実装は別途 `spec/features/` に個別要件ファイルを作成して進める